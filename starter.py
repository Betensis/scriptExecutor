...
class Scripts:
    def __init__(self):
        self.scripts_dir_environment_name = "SCRIPTS_DIR"
    ...
    def execute(self, file: str, executor: str = None) -> None:
        ...
        try:
            if executor:
                if not (
                    code := subprocess.run([f"{executor} {full_path.resolve()}"])
                ):
                    raise FireError(f"script exit with code {code}")
            if not (code := subprocess.run([f"{full_path.resolve()}"])):
                raise FireError(f"script exit with code {code}")
        except OSError as e:
            raise FireError(
                f"Can't execute {file}. {e.strerror}"
            )
